    ...
    def maxCandies(self, status: List[int], candies: List[int], keys: List[List[int]], containedBoxes: List[List[int]], initialBoxes: List[int]) -> int:
        ...
        while to_open := openable.intersection(boxes_remaining):
            for box in to_open:                             # For each openable box
                ...
                boxes_remaining.remove(box)                # Remove the box from our lists (TLE if not from both, even though technically not necessary)
                openable.update(keys[box])                  # Add all new keys to openable
                for new_box in containedBoxes[box]:         
                    boxes_remaining.add(new_box)            # Add all new boxes discovered to boxes_remaining
                    if status[new_box] == 1:                
                        openable.add(new_box)               # and add them to openable as well if they are unlocked
